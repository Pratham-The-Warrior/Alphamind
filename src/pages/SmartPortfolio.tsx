...
import {
  smartPortfolioAssets as portfolioAssets,
  smartPortfolioRecommendations as aiRecommendations
} from '../services/mockData';

export const SmartPortfolio: React.FC = () => {
  const totalValue = portfolioAssets.reduce((sum: number, asset: any) => sum + asset.value, 0);
        ...
        {portfolioAssets.map((asset, index) => (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <span className={`text-xs px-2 py-1 rounded-full ${asset.risk === 'Very High' ? 'bg-red-500/20 text-red-400' :
          ...
          {aiRecommendations.map((rec: any, index: number) => (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <span className={`font-bold ${rec.action === 'Increase' ? 'text-lime-accent' :
